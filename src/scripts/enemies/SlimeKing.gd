...
func _play_anim(anim_name: String) -> void:
	if sprite.sprite_frames and sprite.sprite_frames.has_animation(anim_name):
		...
		# Attack frames are 288 px tall vs 320 px for walk/hurt.
		# With centered=true the bottom lifts 8 local units — push sprite down to compensate.
		sprite.position.y = 8.6666667 if anim_name == "attack" else 0.6666667
...
func _play_hurt() -> void:
	...
	if not _is_dying:
		sprite.modulate = Color(1.0, 0.5, 0.1) if _enraged else Color.WHITE
	await get_tree().create_timer(0.15).timeout
	if not _is_dying:
		_is_hurt = false
...
func die() -> void:
	...
	# Poll physics frames so move_and_slide() runs and is_on_floor() is fresh.
	for _i in 120:
